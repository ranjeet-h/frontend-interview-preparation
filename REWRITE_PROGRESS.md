# Study-Note Rewrite Progress

Updated: 2026-08-26

Tracking overall progress across all interview study sections. Detailed page-by-page tracking is maintained in [`docs/superpowers/plans/2026-08-25-one-topic-rewrite-tracker.md`](docs/superpowers/plans/2026-08-25-one-topic-rewrite-tracker.md).

---

## Completed Sections Summary (532 pages complete)

| Section | Completed Pages | Status |
|---|---:|---|
| **JavaScript Core Concepts** (`src/javascript/concepts/`) | 56 / 56 | Complete |
| **Frontend Fundamentals** (`src/frontend/html/`, `css/`, `web/`, + banks) | 20 / 20 | Complete |
| **React Core Concepts** (`src/react/concepts/`) | 66 / 66 | Complete |
| **React Focused Chapters** (`src/react/*.md`) | 7 / 7 | Complete |
| **Backend Core Concepts** (`src/full-stack/backend/concepts/`) | 39 / 39 | Complete |
| **Backend System Design** (`src/full-stack/backend/system-design/`) | 30 / 30 | Complete |
| **Node.js Core Concepts** (`src/full-stack/backend/nodejs/`) | 30 / 30 | Complete |
| **Express Core Concepts** (`src/full-stack/backend/express/`) | 30 / 30 | Complete |
| **MongoDB Core Concepts** (`src/full-stack/databases/mongodb/`) | 35 / 35 | Complete |
| **Mongoose Core Concepts** (`src/full-stack/backend/mongoose/`) | 30 / 30 | Complete |
| **SQLAlchemy Core Concepts** (`src/full-stack/backend/sqlalchemy/`) | 30 / 30 | Complete |
| **SQL Core Concepts** (`src/full-stack/databases/sql/`) | 40 / 40 | Complete |
| **PostgreSQL Core Concepts** (`src/full-stack/databases/postgresql/`) | 35 / 35 | Complete |
| **MySQL Core Concepts** (`src/full-stack/databases/mysql/`) | 30 / 30 | Complete |
| **SQL Query Practice** (`src/full-stack/databases/sql-query-practice/`) | 30 / 30 | Complete |
| **Total Completed Concept & Practice Pages** | **532** | **100% Done** |

---

## Pending Rewrite Scope

### JavaScript Output Questions (Active Type C Queue)

| Collection | Total Puzzles | Accepted Pages | Remaining |
|---|---:|---:|---:|
| Part 1 | 40 | 5 | 35 |
| Part 2 | 100 | 5 | 95 |
| Part 3 | 203 | 5 | 198 |
| **Total Output Questions** | **343** | **15** | **328** |

### Backend Sections (Pending Rewrite)

| Section | Total Pages | Completed | In-Flight / Assigned | Not Started |
|---|---:|---:|---:|---:|
| API design | 30 | 0 | 0 | 30 |
| Authentication | 30 | 0 | 0 | 30 |
| Coding practice | 30 | 0 | 0 | 30 |
| Deployment | 25 | 0 | 0 | 25 |
| **FastAPI** (in rewrite now) | **50** | **26** | **0** | **24** |
| Full-stack integration | 20 | 0 | 0 | 20 |
| Observability | 25 | 0 | 0 | 25 |
| Performance | 25 | 0 | 0 | 25 |
| Python backend | 30 | 0 | 0 | 30 |
| Queues | 20 | 0 | 0 | 20 |
| Redis | 20 | 0 | 0 | 20 |
| Security | 30 | 0 | 0 | 30 |
| Senior scenarios | 25 | 0 | 0 | 25 |
| Testing | 20 | 0 | 0 | 20 |
| WebSockets | 20 | 0 | 0 | 20 |
| **Total Backend** | **410** | **26** | **0** | **384** |

### MERN Status

| Section | Total Pages | Accepted | In-Flight / Assigned | Not Started |
|---|---:|---:|---:|---:|
| MERN Full-Stack Patterns | 30 | 25 | 5 | 0 |

### Database Sections Status (135/135 Complete)

| Section | Total Pages | Completed | Status |
|---|---:|---|
| **SQL Core** | 40 | 40 | **Complete** |
| **PostgreSQL** | 35 | 35 | **Complete** |
| **MySQL** | 30 | 30 | **Complete** |
| **SQL Query Practice** | 30 | 30 | **Complete** |
| **Total Database** | **135** | **135** | **100% Done** |

---

## Cross-Project Summary

- **Total Completed Concept/System Pages:** **532**
- **Total Completed Output Question Puzzles:** **15**
- **Database Remaining:** **0** (All 135 database pages complete!)
- **Backend Remaining:** **384** (FastAPI 24 + other backend sections 360)
