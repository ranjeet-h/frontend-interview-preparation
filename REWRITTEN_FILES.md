# Study-note rewrite tracker

Updated: 2026-08-25

This file lists the topic pages rewritten during the sequential study-note rewrite. A page is marked `committed` only after its worker rewrite, Luna review/fix cycle, master verification, and target-only commit. Pages marked `review pending` have a worker rewrite in the shared worktree but are not yet accepted.
